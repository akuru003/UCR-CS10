...
void perc(vector <vector <int> > &grid)
{
   ...
   for(int r = 0; r < grid.size(); r++)
   {
      for(int c = 0; c < grid.at(r).size(); c++)
      {
         ...
         {
            if((r - 1 > - 1 ) && (grid.at(r - 1).at(c) == COLOR))
            {
               grid.at(r).at(c) = TO_BE_COLORED;
            }
            else if((r + 1 < grid.size()) && (grid.at(r + 1).at(c) == COLOR))
            {
               grid.at(r).at(c) = TO_BE_COLORED;
            }
            else if((c - 1 > -1) && (grid.at(r).at(c - 1) == COLOR))
            {
               grid.at(r).at(c) = TO_BE_COLORED;
            }  
            else if((c + 1 < grid.at(r).size()) && 
            (grid.at(r).at(c + 1) == COLOR))
            {
               grid.at(r).at(c) = TO_BE_COLORED;
            }
         }
   ...
   for(int r = 0; r < grid.size(); r++)
   {
      for(int c = 0; c < grid.at(r).size(); c++)
      {
         if(grid.at(r).at(c) == TO_BE_COLORED)
         {
            grid.at(r).at(c) = COLOR;
         }
      }
   }
}
